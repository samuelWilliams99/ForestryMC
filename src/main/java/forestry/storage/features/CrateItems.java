...
public class CrateItems {
	...
	public static final FeatureItem<ItemCrated> CRATE = REGISTRY.item(() -> new ItemCrated(null), "crate");
	...
	public static final FeatureItemGroup<ItemCrated, EnumHoneyComb> CRATED_BEE_COMBS = REGISTRY.itemGroup(comb -> new ItemCrated(ApicultureItems.BEE_COMBS.get(comb).asItem()), "crated_bee_comb", EnumHoneyComb.VALUES);
	...
	private static FeatureItem<ItemCrated> register(ItemLike contained, String identifier) {
		FeatureItem<ItemCrated> item = REGISTRY.item(() -> new ItemCrated(contained.asItem()), identifier);
		CRATES.add(item);
		return item;
	}
